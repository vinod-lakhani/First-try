...
def process_csv_no_headers(input_file: str, output_file: str) -> Tuple[int, int]:
    """
    Process the input CSV file (no headers) and create output CSV with valid email rows.
    
    Expected format: Name,Major,School,Email
    ...
    try:
        with open(input_file, 'r', newline='', encoding='utf-8') as infile, \
            ...
            for row in reader:
                ...
                # Extract data from input row (Name,Major,School,Email)
                full_name = row[0] if len(row) > 0 else ''
                major = row[1] if len(row) > 1 else ''
...
def main():
    ...
    """
    if len(sys.argv) != 3:
        print("Usage: python process_graduation_data_fixed.py <input_file.csv> <output_file.csv>")
        print("Example: python process_graduation_data_fixed.py output_with_verified_emails.csv processed_graduates.csv")
        sys.exit(1)
    ...
    total_processed, valid_output = process_csv_no_headers(input_file, output_file)
