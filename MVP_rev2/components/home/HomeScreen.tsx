...
export function HomeScreen({ data }: HomeScreenProps) {
  ...
  const usePlanPayroll = (plan401kEmployeeMonthly > 0.01 || planHsaMonthly > 0.01) && planData;

  const grossIncomeMonthly = getGrossIncomeMonthly(income);
  const employerMatchFromPlan = calculateEmployerMatch(plan401kEmployeeMonthly, grossIncomeMonthly, payrollContributions ?? undefined);
  ...
  const rothSub = longTerm?.subCategories?.find(s => s.key === 'retirement_tax_advantaged');
  const brokerageSub = longTerm?.subCategories?.find(s => s.key === 'brokerage');
  const planBasedCashMTD = planData
    ? (emergencyCat?.amount ?? 0) * paychecksPerMonth + (debtExtraCat?.amount ?? 0) * paychecksPerMonth
      + (rothSub?.amount ?? 0) * paychecksPerMonth + (brokerageSub?.amount ?? 0) * paychecksPerMonth
    : 0;
  ...
  const expectedPayrollSavingsMTD = usePlanPayroll
    ...
    : savingsBreakdown.payrollSavingsMTD;
  const expectedMatchMTD = usePlanPayroll ? employerMatchFromPlan : savingsBreakdown.employerMatchMTD;
