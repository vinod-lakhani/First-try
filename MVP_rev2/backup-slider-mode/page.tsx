...
function SavingsAllocatorContent() {
  ...
  /** User override deltas (chat or sliders). When any set, we're in PROPOSAL mode. */
  ...
  const updateOverrideToValue = (
    ...
    const cur = category === 'ef' ? currentPlan.ef$ : category === 'debt' ? currentPlan.debt$ : category === 'retirementExtra' ? currentPlan.retirementTaxAdv$ : currentPlan.brokerage$;
    const delta = absoluteValue - cur;
    const key = category === 'retirementExtra' ? 'retirementExtraDelta' : `${category}Delta`;
  ...
  // Budget for post-tax categories (ef, debt, retirement, brokerage) — excludes 401k and HSA when engine allocates them
  const postTaxBudgetForRebalance = useMemo(() => {
    if (!savingsBudget || !engineAllocationForScenario) return savingsBudget;
    const preTax401k = engineAllocationForScenario.preTax401k$ ?? 0;
    const hsa = engineAllocationForScenario.hsa$ ?? 0;
    return Math.max(0, savingsBudget - preTax401k - hsa);
  }, [savingsBudget, engineAllocationForScenario]);

  // proposedPlan = engine when no overrides, else engine + overrides. Use engine as base when applying overrides so we preserve 401k, HSA.
  const proposedPlanSnapshot = useMemo((): SavingsPlanSnapshot | null => {
    ...
    if (hasOverrides) return applyOverridesAndRebalance(baseForOverrides, overrides, postTaxBudgetForRebalance);
    return baseForOverrides;
  }, [currentPlan, overrides, savingsBudget, postTaxBudgetForRebalance, hasOverrides, engineSnapshot]);
  ...
  const proposedAllocation = useMemo(() => {
    ...
    const preTax401k$ = engineAllocationForScenario?.preTax401k$ ?? preTaxSavings.traditional401k.monthly;
    return {
  ...
  }, [proposedPlanSnapshot, engineAllocationForScenario, preTaxSavings.traditional401k.monthly]);
  ...
  const allocationComparison = useMemo(() => {
    ...
    const proposed401kEmployee = engineAllocationForScenario?.preTax401k$ ?? 0;
  ...
  }, [currentPlan, proposedPlanSnapshot, preTaxSavings.traditional401k.monthly, engineAllocationForScenario]);
      ...
      <div className="flex-1 overflow-y-auto px-4 py-6">
        <div className="mx-auto w-full max-w-lg space-y-6">
            ...
            <div className="rounded-lg border border-slate-200 dark:border-slate-700 overflow-hidden">
              ...
              {adjustDetailsOpen && (
                <div className="border-t border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-4 space-y-6">
                  {/* Pre-Tax Savings — show proposed values when available (matches proposed plan) */}
                  {(() => {
                    const display401k = engineAllocationForScenario?.preTax401k$ ?? preTaxSavings.traditional401k.monthly;
                    const displayHsa = (proposedPlanSnapshot?.hsa$ ?? engineAllocationForScenario?.hsa$ ?? preTaxSavings.hsa.monthly);
                    const displayMatch = (proposedPlanSnapshot?.match401k$ ?? engineAllocationForScenario?.match401k$ ?? preTaxSavings.employerMatch.monthly);
                    const hasAnyPreTax = display401k > 0 || displayHsa > 0 || displayMatch > 0 || preTaxSavings.employerHSA?.monthly > 0 || baselineState.payrollContributions?.has401k;
                    return hasAnyPreTax && (
              ...
              <div className="space-y-2.5">
                {display401k > 0 && (
                  <div className="flex items-center justify-between text-sm">
                    <span className="text-slate-700 dark:text-slate-300">
                      Traditional 401(k): {preTaxSavings.traditional401k.percent != null && Math.abs(preTaxSavings.traditional401k.monthly - display401k) < 0.01
                        ? `${preTaxSavings.traditional401k.percent.toFixed(1)}%`
                        : '—'}
                    </span>
                    <span className="font-medium text-slate-900 dark:text-white">
                      ~${Math.round(display401k).toLocaleString('en-US')}/mo {display401k !== preTaxSavings.traditional401k.monthly ? '(proposed)' : '(estimated)'}
                    </span>
                  </div>
                )}
                ...
                {displayHsa > 0 && (
                  <div className="flex items-center justify-between text-sm">
                    ...
                    <span className="font-medium text-slate-900 dark:text-white">
                      ~${Math.round(displayHsa).toLocaleString('en-US')}/mo {Math.abs(displayHsa - preTaxSavings.hsa.monthly) > 0.01 ? '(proposed)' : '(estimated)'}
                    </span>
                  </div>
                )}
                ...
                {displayMatch > 0 && (
                  <div className="flex items-center justify-between text-sm">
                    ...
                    <span className="font-medium text-green-600 dark:text-green-400">
                      ~+${Math.round(displayMatch).toLocaleString('en-US')}/mo {Math.abs(displayMatch - preTaxSavings.employerMatch.monthly) > 0.01 ? '(proposed)' : '(estimated)'}
                    </span>
                  </div>
                )}
                {preTaxSavings.employerHSA?.monthly && preTaxSavings.employerHSA.monthly > 0 && (
                  <div className="flex items-center justify-between text-sm">
          ...
          <div className="space-y-6">
            ...
            {/* Adjust allocation — Phase 2A: changes create proposed update until confirmed */}
            <div
            ...
            >
              <h2 className="font-semibold text-slate-900 dark:text-white">Adjust allocation</h2>
              <p className="text-sm text-slate-600 dark:text-slate-400">
                Changes create a proposed update. Nothing is applied until you confirm.
              </p>
              ...
              <div className="mb-6">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div className="flex-1 rounded-lg border border-slate-300 bg-white px-4 py-3 dark:border-slate-600 dark:bg-slate-800">
                    <input
                      type="number"
                      value={Math.round(proposedPlanSnapshot?.ef$ ?? 0)}
                      onChange={(e) => {
                        const value = parseFloat(e.target.value) || 0;
                        updateOverrideToValue('ef', value);
                      }}
                  ...
                  <Button
                    ...
                    onClick={(e) => {
                      ...
                      console.log('[Savings Allocator] Plus button clicked for emergency fund:', {
                        currentAmount: proposedPlanSnapshot?.ef$ ?? 0,
                        savingsBudget,
                        efGap$,
              ...
              <div className="mb-6">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div className="flex-1 rounded-lg border border-slate-300 bg-white px-4 py-3 dark:border-slate-600 dark:bg-slate-800">
                    <input
                      type="number"
                      value={Math.round(proposedPlanSnapshot?.debt$ ?? 0)}
                      onChange={(e) => {
                        const value = parseFloat(e.target.value) || 0;
                        updateOverrideToValue('debt', value);
                      }}
                      className="w-full text-right text-lg font-semibold bg-transparent border-none outline-none"
                      min={0}
                      disabled={totalDebtBalance$ === 0}
                  ...
                  <Button
                    ...
                    onClick={(e) => {
                      ...
                      console.log('[Savings Allocator] Plus button clicked for debt:', {
                        currentAmount: proposedPlanSnapshot?.debt$ ?? 0,
                        savingsBudget,
                        target40Percent: savingsBudget * 0.4,
                      });
                      applyOverride('debt', 50);
                    }}
                    className="h-10 w-10 shrink-0"
                    disabled={totalDebtBalance$ === 0}
                    type="button"
              ...
              <div className="mb-6">
                ...
                <div className="flex items-center gap-2">
                  <Button
                    variant="outline"
                    size="icon"
                    onClick={() => applyOverride('retirementExtra', -50)}
                    className="h-10 w-10 shrink-0"
                  >
                    <Minus className="h-4 w-4" />
                  </Button>
                  <div className="flex-1 rounded-lg border border-slate-300 bg-white px-4 py-3 dark:border-slate-600 dark:bg-slate-800">
                    <input
                      type="number"
                      value={Math.round(proposedPlanSnapshot?.retirementTaxAdv$ ?? 0)}
                      onChange={(e) => {
                        const value = parseFloat(e.target.value) || 0;
                        updateOverrideToValue('retirementExtra', value);
                      }}
                      className="w-full text-right text-lg font-semibold bg-transparent border-none outline-none"
                      min={0}
                    />
                  ...
                  <Button
                    ...
                    onClick={(e) => {
                      ...
                      console.log('[Savings Allocator] Plus button clicked for retirement:', {
                        currentAmount: proposedPlanSnapshot?.retirementTaxAdv$ ?? 0,
                      });
                      applyOverride('retirementExtra', 50);
                    }}
                    className="h-10 w-10 shrink-0"
                    type="button"
                  >
              ...
              <div className="mb-6">
                ...
                <div className="flex items-center gap-2">
                  <Button
                    variant="outline"
                    size="icon"
                    onClick={() => applyOverride('brokerage', -50)}
                    className="h-10 w-10 shrink-0"
                  >
                    <Minus className="h-4 w-4" />
                  </Button>
                  <div className="flex-1 rounded-lg border border-slate-300 bg-white px-4 py-3 dark:border-slate-600 dark:bg-slate-800">
                    <input
                      type="number"
                      value={Math.round(proposedPlanSnapshot?.brokerage$ ?? 0)}
                      onChange={(e) => {
                        const value = parseFloat(e.target.value) || 0;
                        updateOverrideToValue('brokerage', value);
                      }}
                      className="w-full text-right text-lg font-semibold bg-transparent border-none outline-none"
                      min={0}
                    />
                  ...
                  <Button
                    ...
                    onClick={(e) => {
                      ...
                      console.log('[Savings Allocator] Plus button clicked for brokerage:', {
                        currentAmount: proposedPlanSnapshot?.brokerage$ ?? 0,
                      });
                      applyOverride('brokerage', 50);
                    }}
                    className="h-10 w-10 shrink-0"
                    type="button"
                  >
