...
export function generateBoostedPlanAndProjection(
  ...
  const matchNeedMonthly$ = safetyStrategy?.match401kPerMonth$ ?? 0;

  // HSA params (same logic as buildFinalPlanData) for savings allocator
  ...
  const hsaAnnualLimits = { self: 4300, family: 8550, unknown: 4300 };
  const hsaAnnualLimit$ = hsaAnnualLimits[hsaCoverageType as keyof typeof hsaAnnualLimits];
  ...
  const savingsAlloc = allocateSavings({
    ...
    efBalance$,
    highAprDebts,
    matchNeedThisPeriod$: matchNeedMonthly$,
  ...
  // Convert allocator output (monthly) to per-paycheck for paycheck plan (same as app)
  const efPerPaycheck = savingsAlloc.ef$ / paychecksPerMonth;
  const debtPerPaycheck = savingsAlloc.highAprDebt$ / paychecksPerMonth;
  const matchPerPaycheck = savingsAlloc.match401k$ / paychecksPerMonth;
  const hsaPerPaycheck = (savingsAlloc.hsa$ ?? 0) / paychecksPerMonth;
  const retirementPerPaycheck = savingsAlloc.retirementTaxAdv$ / paychecksPerMonth;
  const brokeragePerPaycheck = savingsAlloc.brokerage$ / paychecksPerMonth;

  // Build paycheck plan with savings breakdown (per-paycheck)
  const paycheckPlan: PaycheckPlan = {
    ...
    savingsBreakdown: {
      ef$: efPerPaycheck,
      debt$: debtPerPaycheck,
      match401k$: matchPerPaycheck,
      hsa$: hsaPerPaycheck,
      retirement$: retirementPerPaycheck,
      retirementAcctType: savingsAlloc.routing?.acctType,
      brokerage$: brokeragePerPaycheck,
    },
    categories: [
      {
        name: 'Needs',
...
export function buildFinalPlanData(state: OnboardingState): FinalPlanData {
  ...
  const netWorth6m = simulation.netWorth[5] || currentNetWorth; // Month 6 (index 5)
  const netWorth12m = simulation.netWorth[11] || currentNetWorth; // Month 12 (index 11)
  const netWorth24m = simulation.netWorth[23] || simulation.netWorth[simulation.netWorth.length - 1] || currentNetWorth; // Month 24 (index 23) or last available
  ...
  const netWorthInsight = `Following this plan, your projected net worth is $${netWorth12m.toLocaleString('en-US', { minimumFractionDigits: 0, maximumFractionDigits: 0 })} in 12 months — up from $${currentNetWorth.toLocaleString('en-US', { minimumFractionDigits: 0, maximumFractionDigits: 0 })} today.`;

  // Chart data - use full simulation data for smooth chart
