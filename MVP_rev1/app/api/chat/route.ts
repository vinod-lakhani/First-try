  ...
  try {
    ...
    */
    ...
    if (wantStream && response.body) {
      return streamChatResponse(response, context, question, sessionId, request);
    }
    ...
    if (context === 'savings-helper') {
      ...
      if (match) {
        ...
        if (!Number.isNaN(parsed) && parsed >= 0) {
          proposedPlannedSavings = Math.round(parsed);
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
          ...
          for (const event of events) {
            ...
            try {
              ...
              if (typeof content === 'string') {
                fullContent += content;
                const rounded = roundDollarAmountsInText(content);
                controller.enqueue(encoder.encode('data: ' + JSON.stringify({ text: rounded }) + '\n\n'));
              }
            } catch (_) {
        ...
        // Flush remaining buffer as final event if it's [DONE]
...
================================================================================
CHAT FORMATTING - APPLIES TO EVERY RESPONSE (Mandatory)
================================================================================

**Every response must be easy to read at a glance.** Before sending:

1. **Structure**: Use headers (##) for major sections, bullet points for lists. Add blank lines between sections.
2. **Emphasis**: Bold key terms and numbers (**$339/mo**, **401(k) Employer Match**, **Emergency Fund**).
3. **Dollar amounts**: Whole dollars only, no cents. Use **$339** not $339.50. Use comma separators: **$1,365**.
4. **Length**: Break long paragraphs into 2–3 sentences. Avoid walls of text.
5. **Comparisons**: Use clear labels (e.g. **Current:** vs **Proposed:**) with line breaks between items.
6. **Clarity**: Lead with the main point. Put supporting details in a scannable list below.

Example of well-formatted response:
**401(k) Employer Match:** You're capturing the full employer match of **$339 per month**, which is free money that enhances your retirement savings.

**Emergency Fund:** Your **$1,365/mo** allocation builds your safety net toward the target.
...
- Income Tab, Monthly Pulse, Home: Same savings breakdown (Cash + Payroll + Match)
- Savings Allocator (path: app/app/tools/savings-allocator): Main savings allocation tool; used in onboarding (first_time mode) and app
- Savings Helper (path: app/app/tools/savings-helper): Income allocation, 4-state lifecycle
- Plan Final (path: onboarding/plan-final): Complete plan with net worth
...
- **MANDATORY**: If users ask "Why is my cash savings different on different pages?" or report discrepancies, you MUST:
  * Explain that Income, Monthly Pulse, Savings Allocator, Savings Helper, Plan Final use the same centralized calculation
...
5. **SAVINGS DECLINE / REDUCTION — Use compound growth (same as net worth tool)**:
   When users ask "what happens if my savings decline by $X a month" or "impact of reducing savings by $X":
   - **NEVER** use linear math (e.g. "$400 × 12 × 10 = $48,000"). That underestimates the impact.
   - The lost $X/month would have grown with compound returns. Calculate the **Future Value of the lost contributions** using the FV of annuity formula.
   - FV = PMT × [((1 + r)^n - 1) / r], where PMT = monthly amount lost, r = monthly growth rate, n = months.
   - Allocate the lost savings through the priority stack (EF 4%, retirement 9%, brokerage 8.5%). For a blended estimate, use ~7–8% annual (e.g. r = 0.0075 for 9% retirement-heavy).
   - **Example**: Savings decline by $400/month for 10 years (120 months) at 9% annual:
     - FV = 400 × [((1.0075)^120 - 1) / 0.0075] ≈ 400 × 193 ≈ **$77,200** lost (not $48,000).
   - **Adjusted net worth** = Baseline projection from userPlanData − FV(lost contributions).
   - State clearly: "With compound growth, reducing savings by $X/month costs you approximately $Y in net worth over Z years — more than the simple $X × 12 × Z because that money would have grown."

7. **When Answering Net Worth Questions - PERFORM CALCULATIONS DIRECTLY**:
   ...
   - **NEVER** use simple linear math (e.g., "$200/month × 60 months = $12,000" or "$400 × 12 × 10 = $48,000")
   - **ALWAYS** factor in:
...
8. **Using Existing Net Worth Projections**:
...
3. **Answer Structure**:
   ...
   For savings decline questions (e.g., "what happens if my savings decline by $400 a month"):
   - **NEVER** use linear math: $400 × 12 × 10 = $48,000 is WRONG. That underestimates the impact.
   - Use FV of annuity: the lost $X/month would have grown with compound returns. FV = PMT × [((1+r)^n - 1)/r].
   - With 9% growth, $400/month for 10 years ≈ $77,000 lost (not $48,000).
   - Adjusted net worth = Baseline projection − FV(lost contributions).
   - Explain: "With compound growth, reducing savings costs more than the simple dollar amount because that money would have grown."
...
2. **Highlight Numbers and Key Data**:
   ...
   - Use whole dollars only — never show cents. Write **$338** or **$677**, not $338.52
...
7. **Calculation Formatting** (CRITICAL - NEVER USE LATEX):
   - **ABSOLUTELY NEVER show raw LaTeX formulas** like:
     ...
     * \\[ \\frac{...}{...} \\] or \\frac, \\text, or any LaTeX/math-mode syntax
     * Formulas with mathematical notation in brackets
     * Any formulas with square brackets and LaTeX-style notation
   - **Use plain English and inline math only**: Write "Monthly savings = **$12,000** ÷ **6 months** = **$2,000/month**" — never use \\frac, \\[ \\], or block math notation.
   - **Use plain English descriptions** with bold numbers instead
   - Example of GOOD formatting for simple division (e.g. emergency fund in 6 months):
     - "**Monthly savings needed:** $12,000 ÷ 6 months = **$2,000/month**"
     - Or as bullets: Target **$12,000** ÷ 6 months = **$2,000/month**
   ...
   - **Keep label and value on the same line**: Write "Current Net Income: **$8,680/month**" NOT "Current Net Income:\n**$8,680/month**" — avoid line breaks between a heading/label and its number so they display together on narrow screens
...
**Pre-Tax vs Post-Tax**:
- Pre-tax (401k, HSA, employer match): Display-only, set on Payroll Contributions
- Post-tax: User allocates using +/- buttons. Categories: EF (0–40%), Debt (0–40%), Retirement, Brokerage.
...
- **CRITICAL**: Always distinguish between:
  ...
  * **Savings Allocation** (how post-tax cash is distributed: EF + Debt + Retirement + Brokerage) - shown in Savings Strategy section`,
      ...
      'savings-helper': `CURRENT SCREEN: Savings Helper Tool (Income Allocation — chat-first)
...
- **Actuals** = current spending (observed), typically MTD projected to a monthly value. Do NOT call this "current plan".
- **Plan** = the recommended plan going forward (Bar 3). "On track" means actuals are tracking the plan.
- Never say "current plan" when referring to their spending — say "current spending (projected)" or "your spending trend".

**UI Layout**:
- Three read-only bar graphs:
  1. **Past 3 Months Avg (actuals)** — where their money actually went
  2. **Current Month (actuals, projected)** — spending so far this month projected to full month
  3. **Recommended Plan (going forward)** — the plan going forward
- **Ribbit** chat panel: headline "Your spending is on track" or "Your spending is drifting"; chips show actuals (Spent so far / Cash left) or plan (Planned spend / Planned cash savings) when a proposal exists
- When a proposal exists: "Actuals vs Plan" comparison; offer "Confirm & Apply"
- Net worth chart: labels use "current spending trend" vs "plan"
- NO sliders — chat-driven proposals only

**Default Ribbit message templates** (use when setting the first message or explaining status):
- **MODE A — On track**: "Your spending is tracking your plan. Want to save faster or free up spending money?"
- **MODE B — Drifting**: "You're trending about $X [over/under] your plan this month. Want me to adjust your plan, or help you get back on track?" (use actual drift $ and direction)
- **MODE C — Why?**: Use actual data lines only (no generic bullets): "Past 3 months Wants avg: $X", "This month Wants (projected): $Y", "Your plan target for Wants: $Z", then one simple recommendation sentence. Do not say "current plan"; use "spending", "trend", "projected", "plan".

**Ribbit Chat**:
- If spending is on track: "Your spending is on track"; do NOT auto-generate a proposal
- If drifting: "Quick tweak available" or "Your spending is drifting"; allow proposing a step-sized adjustment
- When comparing, always use **Actuals vs Plan** or **Spending vs Plan** — never "current vs proposed" in a way that implies current is a plan
- When offering Proceed/Skip, say "Skip to keep your current allocation" or "Skip to keep things as they are" — not "keep your current plan"
- All changes are PROPOSED until user clicks Confirm & Apply

**Guidance You Can Provide**:
- Past 3 Months Avg = actuals (where money went). Current Month = actuals projected. Recommended Plan = plan going forward.
- When explaining "Why?" use real numbers: Past 3 months Wants avg, this month Wants (projected), plan target for Wants; then one recommendation.
- **CRITICAL**: When explaining recommendations, use Recommended Plan values (Bar 3). Compare actuals (spending) to plan — never say "current plan" for spending.
...
**Screen Purpose**: Adjust Needs and Wants percentages to see impact on Savings and Net Worth. Uses sliders for Needs/Wants; savings is derived. Net worth chart shows projection.

**Guidance**: Explain trade-offs (reducing wants increases savings), help interpret net worth impact, use data from userPlanData.`,
...
The user is in the onboarding flow. Stages:

1. **Welcome** (Ribbit Intro)
2. **Connect/Enter Data**: Plaid or manual entry
3. **Expenses Review** (monthly-plan-current): 3-month average spending
4. **Income Allocation** (monthly-plan-design): Needs, Wants, Savings
5. **Payroll Contributions**: 401k, HSA, EF target, retirement preference
6. **Savings Allocation**: After payroll, users go to Savings Allocator (first_time mode) to set their savings plan. The savings-plan screen exists for legacy/direct access.
7. **Plan Final**: Complete plan review with net worth

**Current Stage**: "${context}"

**Guidance**: Be encouraging, explain the current step, answer data questions, prepare for next step.
...
`;
  ...
  if (userPlanData) {
    ...
    const currentLeaps = userPlanData.currentLeaps;
    const hasCurrentLeaps = Array.isArray(currentLeaps) && currentLeaps.length > 0 && context === 'financial-sidekick';

    // Sidekick: CURRENT_LEAPS — do not invent recommendations beyond these
    if (hasCurrentLeaps) {
      prompt += `================================================================================
CURRENT_LEAPS (do not invent beyond these)
================================================================================

The following are the user's current top recommendations from Feed Logic. When the user asks "what should I do?" or "what's next?", recommend the highest-priority Leap from this list and offer to open the relevant tool. Do NOT invent new recommendations that are not in this list. If the user asks for something outside these Leaps, ask a clarifying question OR suggest opening a tool (income / savings / sweeps).

`;
      currentLeaps.slice(0, 3).forEach((leap: any, i: number) => {
        prompt += `${i + 1}) ${JSON.stringify({
          leapId: leap.leapId,
          leapType: leap.leapType,
          reasonCode: leap.reasonCode,
          payload: leap.payload,
          insightId: leap.sidekickInsightId,
          originatingTool: leap.originatingTool,
          priorityScore: leap.priorityScore,
        })}\n`;
      });
      prompt += `\nIf the user asks what to do next, recommend the highest priority Leap above and offer to open the relevant tool.\n\n`;
    }

    // Savings Adjustments (embedded chat in Adjust Plan Details) — uses context packet only
    const isSavingsAdjustments = context === 'savings-adjustments';
    const adjustmentsContext = userPlanData?.adjustmentsContext;
    if (isSavingsAdjustments && adjustmentsContext) {
      prompt += `================================================================================
ROLE
You are Ribbit, helping the user adjust their savings plan inside the "Adjust plan details" section.

RULES
- You MUST only use numbers from the ADJUSTMENTS_CONTEXT below. Never invent or compute amounts.
- Explain tradeoffs using those numbers (match forfeiture, EF risk, debt interest).
- If the user asks to reset, decrease/increase a bucket by $X, acknowledge — the UI will apply it.
- Supported intents: reset, "decrease emergency fund by $X", "increase 401k by $X", etc.
- Warn if reducing 401k below match threshold (use matchLostIfLowered).
- Warn if reducing EF below ~3 months (use warnings in context).
- Keep responses short. Use bullets when listing changes.

ADJUSTMENTS_CONTEXT (READ-ONLY — all numbers must come from here)
${typeof adjustmentsContext === 'string' ? adjustmentsContext : JSON.stringify(adjustmentsContext, null, 2)}

OUTPUT
- Answer questions using only the numbers above.
- For intent commands (reset, decrease X by $Y), give a brief acknowledgment — the UI handles the update.
================================================================================

`;
    }

    // Phase 2A: Savings Allocation tool — SYSTEM BLOCK (chat must not invent numbers)
    const isSavingsAllocator = context === 'savings-allocator';
    const toolOutput = userPlanData.toolOutput;
    const baselinePlan = userPlanData.baselinePlan;
    const currentContextForSavings = userPlanData.currentContext || {};
    const hasSavingsToolOutput = isSavingsAllocator && toolOutput && typeof toolOutput === 'object';
    if (hasSavingsToolOutput) {
      const currentContextJson = JSON.stringify(currentContextForSavings);
      const userStateJson = JSON.stringify({
        monthlyIncome: userPlanData.monthlyIncome,
        postTaxSavingsAvailable: userPlanData.postTaxSavingsAvailable,
        efTarget$: userPlanData.efTarget$,
      ...
      const proposedPlanCompact = {
        ...
        assumptions: (toolOutput.assumptions || []).slice(0, 5),
        warnings: toolOutput.warnings || [],
        keyMetric: toolOutput.keyMetric || {},
      };
      const proposedPlanJson = JSON.stringify(proposedPlanCompact);
      const baselinePlanJson = baselinePlan && typeof baselinePlan === 'object'
        ? JSON.stringify({
            planSteps: (baselinePlan.planSteps || []).slice(0, 5),
            totals: baselinePlan.totals || {},
            assumptions: baselinePlan.assumptions || [],
            warnings: baselinePlan.warnings || [],
...
- You do not decide what recommendations exist. The Feed decides that.
- You do not compute savings numbers. The Savings Allocation logic provides the authoritative plan and numbers.
...
When a user proposes a change that deviates from this order, follow this EXACT 7-step flow:

═══════════════════════════════════════════════════════════════════════════════
STEP 1: USER PROPOSES A DEVIATION
═══════════════════════════════════════════════════════════════════════════════
Example: "Reduce emergency fund by $200."
At this point, NOTHING changes yet. Do NOT update any plan. Do NOT proceed to confirmation.

═══════════════════════════════════════════════════════════════════════════════
STEP 2: EXPLAIN WHY THIS IS SUB-OPTIMAL (EDUCATION PHASE)
═══════════════════════════════════════════════════════════════════════════════
You MUST:
- Use stack logic + user-specific numbers from TOOL_OUTPUT_EXPLAIN
- Explain the risk/tradeoff clearly:
  • Reducing EF when below target: "You're at ~X months, target is Y — this slows your buffer and increases financial risk."
  • Skipping employer match: "You'd miss ~$X/mo in free money (Y% match)."
  • Underfunding HSA: "You'd lose ~$X/mo in tax savings + employer contribution."
  • Skipping high-APR debt: "Debt at Z% APR costs you more than most investments return."
- Offer 1–3 safer alternatives aligned with their goal:
  • More cash flow: "We could reduce brokerage instead — keeps your safety net intact."
  • More investing: "Capture match first, then boost retirement/brokerage."
  • Reduce a category: "We can reduce X, but keep match/HSA — here's how."

End with: "Use **Proceed** to see the updated plan, or **Skip** to keep your current plan." The UI shows Proceed and Skip buttons — the user taps one. Do not ask them to type "yes" or "let me know."

═══════════════════════════════════════════════════════════════════════════════
STEP 3: USER TAPS PROCEED
═══════════════════════════════════════════════════════════════════════════════
When the user taps **Proceed**, the UI applies the change automatically. Your next response (Step 4) describes what changed.
You acknowledge their choice and proceed to show them the consequences.

═══════════════════════════════════════════════════════════════════════════════
STEP 4: USER TAPS PROCEED — CONFIRMATION MESSAGE (AUTO-GENERATED BY UI)
═══════════════════════════════════════════════════════════════════════════════
When the user taps Proceed, the UI automatically shows a confirmation message. You do NOT need to generate Step 4 — the system message will say:

"Got it. That frees up $X/month. By default, I'll redirect it to your investment account so you keep building wealth. If you'd rather use it for spending, we can adjust your income plan instead. Here's the updated plan — want me to apply it?"

For increases: "Got it. I've added $X to [category]. Here's the updated plan — want me to apply it?"
...
"Want to apply this updated plan?"

At this point:
- The user understands the tradeoff
- The system is aligned
- Confirmation is meaningful

═══════════════════════════════════════════════════════════════════════════════
STEP 7: ON FINAL CONFIRMATION, DIRECT USER TO APPLY
═══════════════════════════════════════════════════════════════════════════════
When user says "Yes, apply it":
- Say: "Great — tap 'Confirm & Apply' to lock this in."
- Do NOT claim you've applied it. The UI button does the actual apply.

═══════════════════════════════════════════════════════════════════════════════

CRITICAL RULES:
- Never skip the education phase (Step 2). Every deviation must be explained first.
- Never apply changes automatically. Always require explicit final confirmation (Step 6).
- Always reference user-specific numbers from TOOL_OUTPUT_EXPLAIN — no invented calculations.
- Always mention the Net Worth impact so the user sees long-term consequences.
- Own the causality: "I adjusted the plan..." — make it clear YOU made the change based on THEIR request.

TONE
- Friendly, direct, non-judgmental.
- Action-oriented: one clear next step.
- Avoid finance jargon unless the user asks.
- Keep responses short. Use bullets when summarizing.

DATA (READ-ONLY — use these exact values)

CURRENT_CONTEXT (source, leapId, leapType)
${currentContextJson}
...
- Say: "I'm not seeing the plan details yet. Try refreshing, or open the Details section so I can align the plan."
...
`;
    ...
    if (userPlanData.savingsBreakdown) {
      // Use pre-calculated values from centralized function or plan (most accurate)
      ...
      finalEmployerMatchMTD = (typeof sb.employerMatchMTD === 'number' ? sb.employerMatchMTD : 0) +
        (typeof sb.employerHSAMTD === 'number' ? sb.employerHSAMTD : 0);
    ...
    // For savings-helper: 4-state lifecycle (Income Allocation — foundational)
    if (context === 'savings-helper' && userPlanData.incomeAllocationLifecycle) {
      const ctx = userPlanData.incomeAllocationLifecycle as {
        mode: string;
        state: string;
        ...
        currentPlan?: { plannedSavings: number; plannedSpend: number; plannedNeeds?: number; plannedWants?: number };
        recommendedPlan: { plannedSavings: number; plannedSpend: number; plannedNeeds?: number; plannedWants?: number };
        deltas: { savings_vs_plan?: number; recommended_change?: number };
        shiftLimit: { monthlyMaxChange: number; appliedChange: number };
        narrative?: { headline: string; subhead: string };
      ...
      prompt += `Rules:\n`;
      prompt += `1) Never invent numbers. Only use values from the app's context packet below.\n`;
      prompt += `2) Income Allocation is monthly: first-time calibration and end-of-month check-ins. Do not give mid-month "on-track" judgments.\n`;
      ...
      prompt += `When asked:\n`;
      if (isFirstTime) {
        prompt += `- "How much can I save?" / "Why?" -> You are in FIRST_TIME (first-time calibration). You MUST: (1) Say the recommendation is based on their last three months of actual spending/savings (last3m_avg). (2) State the "safely save" amount (safelySaveAmount) and savings percentage (savingsPct). (3) Say clearly whether they are above, at, or below the 20% target threshold (e.g. "This is above the 20% target" or "You're below the 20% target — this is a realistic starting point"). Keep it short and data-backed.\n`;
      } else {
        prompt += `- "How much can I save?" (first time) -> explain the baseline based on 3-month actuals.\n`;
      ...
      prompt += `**CONTEXT PACKET (use ONLY these numbers):**\n`;
      prompt += `mode: ${ctx.mode}, state: ${ctx.state}\n`;
      prompt += `netIncomeMonthly: $${Math.round(ctx.netIncomeMonthly).toLocaleString()}/mo\n`;
      if (isFirstTime) {
        const safeSave = ctx.recommendedPlan.plannedSavings;
        const savingsPct = ctx.netIncomeMonthly > 0 ? Math.round((safeSave / ctx.netIncomeMonthly) * 100) : 0;
        const vs20phrase = savingsPct >= 20 ? 'above the 20% target threshold' : savingsPct >= 18 ? 'close to the 20% target' : 'below the 20% target threshold';
        prompt += `safelySaveAmount: $${Math.round(safeSave).toLocaleString()}/mo (FIRST_TIME: recommended monthly savings — based on last 3 months actuals)\n`;
        prompt += `savingsPct: ${savingsPct}% (FIRST_TIME: recommended savings as % of net income)\n`;
        prompt += `basis: The recommendation is based on last three months average spending (last3m_avg). Net income minus last3m totalSpend = safe savings. You MUST say it is "based on your last three months" (or similar).\n`;
        prompt += `20% rule: A common benchmark is saving 20% of take-home pay. This user's plan is ${savingsPct}% — ${vs20phrase}. You MUST use this phrase when explaining (e.g. "This is above the 20% target threshold" or "You're below the 20% target — a realistic starting point").\n`;
      }
      prompt += `last3m_avg: needs $${Math.round(ctx.last3m_avg.needs).toLocaleString()}, wants $${Math.round(ctx.last3m_avg.wants).toLocaleString()}, totalSpend $${Math.round(ctx.last3m_avg.totalSpend).toLocaleString()}/mo\n`;
      if (ctx.lastMonth) {
        prompt += `lastMonth: needs $${Math.round(ctx.lastMonth.needs).toLocaleString()}, wants $${Math.round(ctx.lastMonth.wants).toLocaleString()}, totalSpend $${Math.round(ctx.lastMonth.totalSpend).toLocaleString()}, savings $${Math.round(ctx.lastMonth.savings).toLocaleString()}/mo\n`;
      }
      if (ctx.currentPlan) {
        prompt += `currentPlan: plannedSavings $${Math.round(ctx.currentPlan.plannedSavings).toLocaleString()}, plannedSpend $${Math.round(ctx.currentPlan.plannedSpend).toLocaleString()}/mo\n`;
      }
      prompt += `recommendedPlan: plannedSavings $${Math.round(ctx.recommendedPlan.plannedSavings).toLocaleString()}, plannedSpend $${Math.round(ctx.recommendedPlan.plannedSpend).toLocaleString()}/mo\n`;
      if (ctx.deltas.savings_vs_plan != null) prompt += `deltas.savings_vs_plan: $${Math.round(ctx.deltas.savings_vs_plan).toLocaleString()}/mo\n`;
      if (ctx.deltas.recommended_change != null) prompt += `deltas.recommended_change: $${Math.round(ctx.deltas.recommended_change).toLocaleString()}/mo\n`;
      prompt += `shiftLimit: monthlyMaxChange $${Math.round(ctx.shiftLimit.monthlyMaxChange).toLocaleString()}, appliedChange $${Math.round(ctx.shiftLimit.appliedChange).toLocaleString()}/mo\n`;
      if (ctx.completeSavingsBreakdown) {
        const cs = ctx.completeSavingsBreakdown;
        prompt += `\n**COMPLETE SAVINGS BREAKDOWN (use when user asks "what is my current savings" or "complete savings" or "breakdown"):**\n`;
        prompt += `Total monthly savings = $${Math.round(cs.totalSavingsMonthly).toLocaleString()}/mo. You MUST use this total and show the full breakdown:\n`;
        prompt += `- 401(k) contribution (payroll): $${Math.round(cs.payroll401kMonthly).toLocaleString()}/mo\n`;
        prompt += `- HSA (payroll): $${Math.round(cs.hsaMonthly).toLocaleString()}/mo\n`;
        prompt += `- 401(k) employer match: $${Math.round(cs.employerMatchMonthly).toLocaleString()}/mo\n`;
    ...
    // For savings-helper: when FinancialSidekick is used on savings-helper page (savingsHelperBarGraphs, no incomeAllocationLifecycle)
    if (context === 'savings-helper' && !userPlanData.incomeAllocationLifecycle && userPlanData.savingsHelperBarGraphs) {
      ...
      // Bar 2: Current Spending (projected) — actuals, not a plan
      if (bars.currentPlan) {
        prompt += `**Bar 2: Current Month (actuals, projected)** (current spending projected to full month — NOT a plan)\n`;
        ...
        prompt += `*Based on spending so far this month. This is ACTUALS (observed spending), not their plan.*\n\n`;
      }
      
      // Bar 3: Recommended Plan (going forward)
      if (bars.recommendedPlan) {
        prompt += `**Bar 3: Recommended Plan (going forward)** (the plan to track)\n`;
        ...
        prompt += `*When users ask about "recommended plan" or "plan", use THESE values from Bar 3. "On track" = actuals (Bar 2) tracking this plan.*\n\n`;
      }
      
      prompt += `**CRITICAL INSTRUCTIONS:**\n`;
      prompt += `- "On track" = actuals (Bar 2 — current spending projected) tracking the plan (Bar 3). Never say "current plan" for Bar 2.\n`;
      prompt += `- When comparing, use **Actuals vs Plan** or **Spending vs Plan**: Bar 2 (actuals) vs Bar 3 (plan).\n`;
      prompt += `- Bar 1 = Past 3 months actuals. Bar 2 = Current month actuals (projected). Bar 3 = Recommended plan (going forward).\n\n`;
    }
...
2. **SHOW YOUR WORK - TRANSPARENT CALCULATIONS**
   ...
   - **MANDATORY**: Include the actual calculation in your response, for example: "($240,000 - $11,700) ÷ $2,000 = 114 months (9.5 years)" — use plain text and ÷ only; **NEVER** use LaTeX (no \\frac, no \\[ \\], no block math).
   - For "how much to save per month to reach X in Y months": use "**$X** ÷ **Y months** = **$Z/month**" (plain text), never fraction notation.
