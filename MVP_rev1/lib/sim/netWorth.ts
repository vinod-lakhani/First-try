...
export interface MonthlyPlan {
  ...
  ef$: number;
  highAprDebt$: number;
  /** User's pre-tax 401(k) contribution (payroll) — goes to retirement */
  preTax401k$?: number;
  /** Employer 401(k) match — goes to retirement */
  match401k$: number;
  /** User's HSA contribution (payroll) — goes to HSA */
  hsa$?: number;
  /** Employer HSA contribution — goes to HSA */
  employerHsa$?: number;
  /** Post-tax tax-advantaged (e.g. Roth) — goes to retirement */
...
/**
 * Returns a copy of monthly plan with total contribution reduced by |delta$| (e.g. -200 = $200 less per month).
 * Reduces brokerage first, then retirementTaxAdv, then ef$, so the sim sees exactly $X less inflow each month.
 */
function applyMonthlyDelta(plan: MonthlyPlan, delta$: number): MonthlyPlan {
  const reduceBy = Math.abs(delta$);
  if (reduceBy <= 0) return { ...plan };
  let remaining = reduceBy;
  const brokerage$ = Math.max(0, (plan.brokerage$ ?? 0) - remaining);
  remaining -= (plan.brokerage$ ?? 0) - brokerage$;
  if (remaining <= 0) return { ...plan, brokerage$ };
  const retirementTaxAdv$ = Math.max(0, (plan.retirementTaxAdv$ ?? 0) - remaining);
  remaining -= (plan.retirementTaxAdv$ ?? 0) - retirementTaxAdv$;
  if (remaining <= 0) return { ...plan, brokerage$, retirementTaxAdv$ };
  const ef$ = Math.max(0, (plan.ef$ ?? 0) - remaining);
  return { ...plan, brokerage$, retirementTaxAdv$, ef$ };
}

/**
 * Run the same scenario but with $X less contributed each month.
 * monthlyDelta$ &lt; 0 = undersave (e.g. -200 → $200 less per month). Proposed = baseline - |monthlyDelta$| every month.
 */
export function simulateScenarioWithMonthlyDelta(input: ScenarioInput, monthlyDelta$: number): ScenarioSeries {
  const reduceBy = monthlyDelta$ <= 0 ? Math.abs(monthlyDelta$) : 0;
  if (reduceBy <= 0) return simulateScenario(input);
  const adjustedPlan = input.monthlyPlan.map((p, i) => ({
    ...applyMonthlyDelta(p, reduceBy),
    monthIndex: i,
  }));
  return simulateScenario({
    ...input,
    monthlyPlan: adjustedPlan,
  });
}
...
export function simulateScenario(input: ScenarioInput): ScenarioSeries {
  ...
  const firstPlan = input.monthlyPlan[0];
  const hasHsaContributions = (firstPlan?.hsa$ ?? 0) + (firstPlan?.employerHsa$ ?? 0) > 0;
  ...
  for (let t = 0; t < months; t++) {
    ...
    // 1) Apply growth (see growth-rates table above)
    ...
    // 2) Inflows from plan (pre-tax 401k, match, HSA, post-tax retirement, brokerage, cash)
    ...
    // 4) Cash outflows — only debt payments (needs/wants are paid from income, not from emergency fund)
    // The emergency fund is a savings buffer you don't touch for regular expenses. Needs and wants
    // are paid from your paycheck before it hits savings. Without this fix, cash would deplete to $0
    // once EF target is reached (ef$ stops, but we were subtracting needs+wants from cash).
    // Note: redirectedMinPayments$ = paid-off debts' min payments, redirected to brokerage (not cash outflow)
    const totalOutflows = round2(totalMinPayment + extraPaid);
