 ...
 * Categories (per Financial Sidekick spec): Leaps → Risk Alerts → Clarity → Insights.
 * Monthly Pulse is no longer included in the feed.
 */
export function buildFeed(snapshot: UserSnapshot): FeedCard[] {
  const cards: FeedCard[] = [];
  const now = new Date().toISOString();

  // 1. Risk Alerts (priority 1)
  ...
  // Sort cards: Leaps (priority 2) first, then Risk Alerts (1), then Clarity/Progress (3), then Education (5)
  cards.sort((a, b) => {
