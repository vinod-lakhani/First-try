...
interface NetWorthChartProps {
  ...
  /** Optional custom labels for primary and baseline series (e.g. "Planned net worth", "Based on current saving") */
  seriesLabels?: { primary?: string; baseline?: string };
}
...
export function NetWorthChart({
  ...
  const { currentValue, change, changePct, deltaFromBaseline, deltaPctFromBaseline, chartData } = useMemo(() => {
    ...
    if (timeRange !== 'ALL') {
      ...
      // Show the first N months: point 0 = Today (opening), then end of month 1, 2, ... N
      // So "1Y" = Today + 12 months = 13 points (indices 0..12)
      const rangeEnd = Math.min(monthsToShow + 1, totalMonths);
    ...
    const baselineLabel = seriesLabels?.baseline ?? 'Current plan';
    const primaryLabel = seriesLabels?.primary ?? (baselineNetWorth ? 'Proposed plan' : 'Net Worth');

    // Add baseline series if provided (e.g. "Based on current saving" or "Current plan")
    if (baselineNetWorth && baselineNetWorth.length > 0) {
      datasets.push({
        label: baselineLabel,
    ...
    // Add primary series (e.g. "Planned net worth" or "Proposed plan")
    datasets.push({
      label: primaryLabel,
  ...
  }, [labels, netWorth, assets, liabilities, timeRange, baselineNetWorth, seriesLabels]);
  ...
  useEffect(() => {
    ...
    import('chart.js/auto').then((ChartModule) => {
      ...
      const primaryLabel = chartData.primaryLabel ?? 'Net Worth';
      const baselineLabel = chartData.baselineLabel ?? 'Current plan';
      const filteredNetWorth = chartData.datasets.find((d: any) => d.label === primaryLabel)?.data || [];
      const filteredBaseline = chartData.datasets.find((d: any) => d.label === baselineLabel)?.data || [];
            ...
            tooltip: {
              ...
              callbacks: {
                ...
                label: function (context: any) {
                  ...
                  } else if (datasetLabel === baselineLabel) {
                  ...
                  // Add delta for primary series when baseline exists
                  if (datasetLabel === primaryLabel && filteredBaseline.length > 0) {
    ...
    <div className="w-full overflow-hidden rounded-lg bg-white shadow-sm">
      ...
      <div className="border-b border-slate-200 bg-white px-5 py-6 dark:border-slate-700 dark:bg-slate-800">
        ...
        <p className="mb-2 text-xs text-slate-500 dark:text-slate-400">
          {timeRange === '1Y' ? 'At end of 1Y' : timeRange === '5Y' ? 'At end of 5Y' : timeRange === '10Y' ? 'At end of 10Y' : timeRange === '20Y' ? 'At end of 20Y' : timeRange === '40Y' || timeRange === 'ALL' ? 'At end of 40Y' : 'Net worth'}
        </p>
          ...
          {baselineNetWorth && baselineNetWorth.length > 0 && (
            ...
            >
              ...
              <span>
                ...
                ({deltaFromBaseline >= 0 ? '+' : ''}{Math.abs(deltaPctFromBaseline).toFixed(2)}%) vs {seriesLabels?.baseline ?? 'Current plan'}
              </span>
            </div>
          )}
        </div>
